...
## Parties
- **Producer:** `ccd` (the Cloud Code Daemon, L5) — publisher of the `ccd.*`
  EventType catalog.
...
## Purpose
CCD emits an agent-lifecycle / run / usage / budget / escalation event stream
...
`ccd.*` topic prefix over `pubsub-protocol`** (the same collapse
`inference-events`/`gc-events` adopt). This file owns only CCD's catalog table
...
"PROPOSED by the ccd Component Designer — pending Decomposer / operator
confirmation" marker is **STRUCK**. The edge is CONFIRMED live and in scope
(wave2-plan §5 flag #6; ccd.md batch-6 pass). Batch designs (`dashboard-serving`,
`dashboard`) already assume it; this contract confirms it. See Reconciliation
notes.

## Schema

CCD is a plain `pubsub-protocol` publisher. It publishes
`types::pubsub::Envelope<types::event::Event<CcdEventPayload>>` frames on topics
under the reserved prefix `ccd`. Topic convention is `ccd/<node_id>/<domain>`
...
```rust
// types::ccd — the CcdEventPayload catalog (P in Event<P>)
// event_type identifiers CCD guarantees to emit:
//   "ccd.agent.started" | "ccd.agent.turn_completed" | "ccd.agent.report_emitted"
//   | "ccd.agent.state_changed" | "ccd.agent.exited"
//   | "ccd.budget.throttled" | "ccd.budget.deferred" | "ccd.limit.observed"
//   | "ccd.escalation.received" | "ccd.escalation.investigating"
pub enum CcdEventPayload {
    ...
    Escalation {                       // mirror of a ccd-escalation receipt, for observability
        escalation_id: Uuid,
...
dashboard reads for its per-node grid — CCD never invents or re-stamps a
`node_id`.

**Subscriber contract (mesh side).** `dashboard-serving` opens ONE subscription
per node with `TopicFilter::Prefix("ccd")` and folds every `Delivery` into the
`GET /events` fan-out unparsed (the relay never inspects `payload`). A focused
per-agent view uses `TopicFilter::Prefix("ccd/<node>/agent")`.
...
- CCD itself owns no request/response error type on this edge (`CcdError` is for
...
(payload-opaque decoupling: a new `ccd.*` event type crosses an older daemon
untouched).
- **Additive-safe:** new `event_type` identifiers; new `CcdEventPayload` arms
...
- **Breaking:** removing/renaming a published `event_type`, changing an existing
  arm's field types, or changing the topic-prefix reservation `ccd`.
- The `Envelope.v` / `Event` shape is governed by `types`' wire discipline
  (guardrail 4); CCD publishes at the current `v` and never straddles two.
...
- **The "proposed, pending confirmation" marker is resolved to CONFIRMED.** Both
  ...
  `dashboard.md` (batch 6) both already build against a `ccd.*` stream as a peer
  of `inference.*`/`gc.*` — striking it would orphan two authored designs;
  (b) INTENT #68 makes CCD a first-class observable service with its own usage
  ...
  designs already consumed it and INTENT #68/#16 (clean agent interface, CCD owns
  its data) implies it.
- **Re-grounded from a WS surface to a pubsub catalog.** Deviation from the
  ...
  by `dashboard-feed`'s `ANY /api/nodes/:id/ccd/*` same-origin proxy.
- **`AgentEvent` is owned by `agent-management`, not duplicated here.** This
  contract references it as the `CcdEventPayload::Agent` arm so the lifecycle
  vocabulary has a single home.

## Example data
A Claude-Code investigation agent runs on **macbook** for project **demo**. CCD
publishes on topic `ccd/macbook/agent` an `Envelope<Event<CcdEventPayload>>`:

```json
{
  "v": 1,
  "envelope_id": "6f1c-a1",
  "topic": "ccd/macbook/agent",
  "published_at": "2026-07-19T18:04:11Z",
  "provenance": { "origin_node": "macbook", "origin_service": "ccd",
                  "emitted_at": "2026-07-19T18:04:11Z",
                  "correlation_id": "c0-demo-run-1" },
  "payload": {
    "event_id": "e-8801",
    "event_type": "ccd.agent.turn_completed",
...
A budget event on the same node (topic `ccd/macbook/budget`,
`event_type: "ccd.budget.throttled"`):
...
`dashboard-serving` on macbook subscribes `Prefix("ccd")`, relays both frames
unparsed to the browser, which renders them on the CCD panel keyed by
`origin_node = "macbook"`. (For the escalation-received event that composes with
this stream, see `ccd-escalation.md` — same `correlation_id` `c0-demo-run-1`.)
