...
## Parties
cc / future `agents`  ->  inference (api, via `v1-completion-api`)

- **inference (api)** is the terminus — the same `/v1/` completion surface as
  `v1-completion-api`. **No second surface is introduced.**
- **cc** is the caller **and** the meter — non-Claude-Code agents make
  completions through `/v1/`, and cc records the usage into its own ledger.
...
2. The edge is **reserved for the future `agents` umbrella's non-CC agent types**,
   ...
   agents reuse `v1-completion-api` (the fleet-facing surface); cc meters that
   usage into the same `usage_records` ledger `spend` reads (pull-shaped).

> **Friction-round 3 narrowing (2026-07-20, INTENT #137):** custom agents do
> NOT route through cc — "custom agents might call OpenRouter, might call the
> inference tool — they're not necessarily going to call cc." cc is for the
> full Claude Code agent shape only. This edge's cc-as-mandatory-meter
> framing is therefore weakened: it survives as an **optional** metering
> surface at most; custom agents' default egress is direct
...
**Metering-shaped view (cc-internal, from the completion response usage block):**

```rust
// types::cc — cc records one row per metered completion (rough sketch)
struct UsageRecord {
    thread: Option<ThreadId>,          // links to a cc thread (and optionally project/env)
...
`StreamEvent`). No inference-side change; cc just reads the response it already
gets.

## Error cases
- Reuses `v1-completion-api`'s error surface entirely (404 / 409 / 422 / 500
  terminus, 502/503 router). cc adds no inference-facing errors.
- `CcError::Inference(String)` — stringified failure on the future agents path
  only (never a Claude Code failure; never becomes an inference type).
- Warm-start: no ledger data yet to attribute a call → cc records the row anyway
...
- **Additive-safe:** new `UsageRecord` fields (cc-internal), new metered surfaces.
- **Breaking:** authoring a Claude-Code-specific completion surface — **forbidden;
  there is none** (conformance).

## Reconciliation notes

1. **Shape vs wave-1 — CONFIRMED thin/reserved (cc wins; the only proposer).**
   Wave-1's stub framed `llm-calls` as "cc/agents make LLM calls through the
   local inference node's `/v1/` API." cc.md refines this: keep it a **thin
   ...
   reconcile — this file records cc's reservation + metering shape and defers
...
(non-Claude-Code) agent under the `agents` umbrella, running in a cc thread on
`macbook`, makes a completion via `/v1/` — the router forwards it to `pi` exactly
as in `v1-completion-api` (completion `7f3c1a90-2b4e-4c1a-9e21-0a1b2c3d4e5f`). cc
meters the result:

```jsonc
// cc usage_records row derived from the /v1/completions/:id/result usage block
{
...
// `spend` later PULLS this row (spend-cc) for the demo project's cost rollup.
```
